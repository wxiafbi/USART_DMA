 ...
 * $Revision:    V1.01
 *  
 * Driver:       Driver_UART1, Driver_UART2, Driver_UART3,
 *               Driver_UART4, Driver_UART5 
 * Configured:   via RTE_Device.h configuration file 
 * Project:      Middleware UART Driver for ST STM32F10x
 * ---------------------------------------------------------------------- 
...
#define ARM_UART_DRV_VERSION ARM_DRV_VERSION_MAJOR_MINOR(1,01)   /* driver version */
...
typedef struct {
  const ARM_UART_CAPABILITIES  *capabilities;         // Capabilities
        ...
        AFIO_REMAP              pin_remap;            // Pin AF remap identifier
  ...
  const DMA_INFO               *dma_tx;               // Transmit DMA configurationn
...
#ifndef __USART1_TX_BUFFER_SIZE
#define __USART1_TX_BUFFER_SIZE         128  // Buffer size must be 2^n
#endif

#ifndef __USART1_RX_BUFFER_SIZE
#define __USART1_RX_BUFFER_SIZE         128  // Buffer size must be 2^n
#endif

#ifndef __USART1_FLOW_CONTROL_THRESHOLD
#define __USART1_FLOW_CONTROL_THRESHOLD  16  // Buffer size must be 2^n
#endif

/* USART1 Capabilities */
static const ARM_UART_CAPABILITIES USART1_Capabilities = {
  __USART1_TX_BUFFER_SIZE,          // Transmit Buffer Size
  __USART1_RX_BUFFER_SIZE,          // Receive Buffer Size
  0,                                // Receive character timeout event
...
static const DMA_INFO USART1_DMA_TX = {
  DMAx_CHANNELy(RTE_USART1_TX_DMA_NUMBER, RTE_USART1_TX_DMA_CHANNEL),
...
/* USART1 Data Info */
// USART Buffer size must be 2^n
struct {
  uint8_t  tx[__USART1_TX_BUFFER_SIZE];
  uint8_t  rx[__USART1_RX_BUFFER_SIZE];
} UART1_Buffer;

static UART_INFO USART1_Info;

/* USART1 Resources */
static UART_RESOURCES USART1_Resources = {
  &USART1_Capabilities,
  ...
  UART1_Buffer.tx,
  __USART1_TX_BUFFER_SIZE,
  UART1_Buffer.rx,
  __USART1_RX_BUFFER_SIZE,
  __USART1_FLOW_CONTROL_THRESHOLD,
#if (RTE_USART1_TX_DMA != 0)
  &USART1_DMA_TX,
#else
  NULL,
...
#ifndef __USART2_TX_BUFFER_SIZE
#define __USART2_TX_BUFFER_SIZE         128  // Buffer size must be 2^n
#endif

#ifndef __USART2_RX_BUFFER_SIZE
#define __USART2_RX_BUFFER_SIZE         128  // Buffer size must be 2^n
#endif

#ifndef __USART2_FLOW_CONTROL_THRESHOLD
#define __USART2_FLOW_CONTROL_THRESHOLD  16  // Buffer size must be 2^n
#endif

/* UART2 Capabilities */
static const ARM_UART_CAPABILITIES USART2_Capabilities = {
  __USART2_TX_BUFFER_SIZE,          // Transmit Buffer Size
  __USART2_RX_BUFFER_SIZE,          // Receive Buffer Size
  0,                                // Receive character timeout event
#if (RTE_USART2_HW_FLOW != 0)
  1,                                // CTS Line available
  1,                                // Signal CTS change event
#else
  0,                                // CTS Line available
...
static const DMA_INFO USART2_DMA_TX = {
  DMAx_CHANNELy(RTE_USART2_TX_DMA_NUMBER, RTE_USART2_TX_DMA_CHANNEL),
...
/* UART2 Semaphores */
osSemaphoreDef(USART2_Semaphore);

/* UART2 Mutex */
osMutexDef(USART2_Mutex_write);
osMutexDef(USART2_Mutex_read);


/* UART2 Data Info */
// UART Buffer size must be 2^n
struct {
  uint8_t  tx[__USART2_TX_BUFFER_SIZE];
  uint8_t  rx[__USART2_RX_BUFFER_SIZE];
} UART2_Buffer;

static UART_INFO USART2_Info;

/* UART2 Resources */
static UART_RESOURCES USART2_Resources = {
  &USART2_Capabilities,
  ...
  UART2_Buffer.tx,
  __USART2_TX_BUFFER_SIZE,
  UART2_Buffer.rx,
  __USART2_RX_BUFFER_SIZE,
  __USART2_FLOW_CONTROL_THRESHOLD,
#if (RTE_USART2_TX_DMA != 0)
  &USART2_DMA_TX,
#else
  NULL,
#endif
  osSemaphore(USART2_Semaphore),
  osMutex(USART2_Mutex_write),
  osMutex(USART2_Mutex_read),
  &USART2_Info
};
#endif
...
#ifndef __USART3_TX_BUFFER_SIZE
#define __USART3_TX_BUFFER_SIZE         128  // Buffer size must be 2^n
#endif

#ifndef __USART3_RX_BUFFER_SIZE
#define __USART3_RX_BUFFER_SIZE         128  // Buffer size must be 2^n
#endif

#ifndef __USART3_FLOW_CONTROL_THRESHOLD
#define __USART3_FLOW_CONTROL_THRESHOLD  16  // Buffer size must be 2^n
#endif

/* USART3 Capabilities */
static const ARM_UART_CAPABILITIES USART3_Capabilities = {
  __USART3_TX_BUFFER_SIZE,          // Transmit Buffer Size
  __USART3_RX_BUFFER_SIZE,          // Receive Buffer Size
  0,                                // Receive character timeout event
...
static const DMA_INFO USART3_DMA_TX = {
  DMAx_CHANNELy(RTE_USART3_TX_DMA_NUMBER, RTE_USART3_TX_DMA_CHANNEL),
...
/* USART3 Semaphores */
osSemaphoreDef(USART3_Semaphore);

/* USART3 Mutex */
osMutexDef(USART3_Mutex_write);
osMutexDef(USART3_Mutex_read);


/* UART3 Data Info */
// UART Buffer size must be 2^n
struct {
  uint8_t  tx[__USART3_TX_BUFFER_SIZE];
  uint8_t  rx[__USART3_RX_BUFFER_SIZE];
} UART3_Buffer;

static UART_INFO USART3_Info;

/* UART3 Resources */
static UART_RESOURCES USART3_Resources = {
  &USART3_Capabilities,
  ...
  UART3_Buffer.tx,
  __USART3_TX_BUFFER_SIZE,
  UART3_Buffer.rx,
  __USART3_RX_BUFFER_SIZE,
  __USART3_FLOW_CONTROL_THRESHOLD,
#if (RTE_USART3_TX_DMA != 0)
  &USART3_DMA_TX,
#else
  NULL,
#endif
  osSemaphore(USART3_Semaphore),
  osMutex(USART3_Mutex_write),
  osMutex(USART3_Mutex_read),
  &USART3_Info
};
#endif
...
static const DMA_INFO UART4_DMA_TX = {
  DMAx_CHANNELy(RTE_UART4_TX_DMA_NUMBER, RTE_UART4_TX_DMA_CHANNEL),
...
#define __UART5_FLOW_CONTROL_THRESHOLD  16  // Buffer size must be 2^n
#endif
...
/* UART Resources */
static UART_RESOURCES *ptr_UART[5] = {
#if (RTE_USART1 != 0)
  &USART1_Resources,
#else
  NULL,
...
static ARM_UART_STATUS UART_Initialize (ARM_UART_SignalEvent_t cb_event,
  ...
  *ptr_uart->reset_reg &= ~ptr_uart->reset_mask;
...
#if DMA_TX
  if (ptr_uart->dma_tx) {
    DMA_ChannelInitialize(ptr_uart->dma_tx->dma, ptr_uart->dma_tx->channel);
  }
#endif
  ...
  GPIO_AFConfigure(ptr_uart->pin_remap); 
...
static ARM_UART_STATUS UART_Uninitialize (UART_RESOURCES *ptr_uart) {
  ...
  *ptr_uart->clock_reg &= ~ptr_uart->clock_mask;
  ...
  if (ptr_uart->dma_tx) {
    DMA_ChannelUninitialize(ptr_uart->dma_tx->dma, ptr_uart->dma_tx->channel);
  }

  GPIO_PortClock                (ptr_uart->pin_tx.port, true);
  
  // Configure TX Pin
  GPIO_PinConfigure    (ptr_uart->pin_tx.port,
                        ptr_uart->pin_tx.num,
                        GPIO_IN_PULL_UP,
                        GPIO_MODE_OUT10MHZ);
  ...
  if (ptr_uart->hw_flow == true) {
    ...
    GPIO_PinConfigure(ptr_uart->pin_cts.port,
                      ptr_uart->pin_cts.num,
                      GPIO_IN_PULL_UP,
                      GPIO_MODE_OUT10MHZ);
    ...
    GPIO_PinConfigure  (ptr_uart->pin_rts.port,
                        ptr_uart->pin_rts.num,
                        GPIO_IN_PULL_UP,
                        GPIO_MODE_OUT10MHZ);
  }
  
  if (ptr_uart->uart == USART1)
    GPIO_AFConfigure(AFIO_USART1_NO_REMAP); 
  else if (ptr_uart->uart == USART2)
    GPIO_AFConfigure(AFIO_USART2_NO_REMAP);
  else if (ptr_uart->uart == USART3)
    GPIO_AFConfigure(AFIO_USART3_NO_REMAP); ;
...
static ARM_UART_STATUS UART_PowerControl (ARM_POWER_STATE state, UART_RESOURCES *ptr_uart) {
  ...
  if (state == ARM_POWER_OFF) {
    ...
    *ptr_uart->clock_reg &= ~ptr_uart->clock_mask;
    ...
    // Disable DMA Streams
    if (ptr_uart->dma_tx) {
      DMA_ChannelDisable(ptr_uart->dma_tx->ptr_channel);
    }
...
static ARM_UART_STATUS UART_Configure (uint32_t               baudrate,
  ...
  // Disable UART transmiter and receiver
  ...
  // If inside +/- 2% tolerance, baudrate configured correctly
  ...
  if (ptr_uart->dma_tx == NULL) {
    ...
    if (osSemaphoreWait(ptr_uart->info->semaphore, 0) > 0) {
      ...
      if ((ptr_uart->info->tx.empty == false) || ((ptr_uart->info->tx.empty == true) && val != 0)) {
        ...
        do {
          if (DMA_ChannelStatus(ptr_uart->dma_tx->ptr_channel) == false) break;
        } while ((osKernelSysTick() - tick) < osKernelSysTickMicroSec(1000));

        if ((DMA_ChannelStatus(ptr_uart->dma_tx->ptr_channel) == false)) {
          DMA_ChannelConfigure(
            ptr_uart->dma_tx->ptr_channel,
          ...
          DMA_ChannelEnable (ptr_uart->dma_tx->ptr_channel);
...
#if DMA_TX
/**
  \fn          void UART_DMA_TxChannelEvent (uint32_t event, UART_RESOURCES *ptr_uart)
  \brief       UART transmit DMA Channel Event.
  \param[in]   event     DMA Channel event
  \param[in]   ptr_uart  Pointer to UART resources
*/
static void UART_DMA_TxChannelEvent (uint32_t event, UART_RESOURCES *ptr_uart) {
...
#if (RTE_USART1 != 0)
#if (RTE_USART1_TX_DMA != 0)
  void DMAx_CHANNELy_EVENT(RTE_USART1_TX_DMA_NUMBER, RTE_USART1_TX_DMA_CHANNEL) (uint32_t event) {
    UART_DMA_TxChannelEvent(event, ptr_UART[0]);
  }
#endif
...
#if (RTE_USART2 != 0)
#if (RTE_USART2_TX_DMA != 0)
  void DMAx_CHANNELy_EVENT(RTE_USART2_TX_DMA_NUMBER, RTE_USART2_TX_DMA_CHANNEL) (uint32_t event) {
    UART_DMA_TxChannelEvent(event, ptr_UART[1]);
  }
#endif
#endif
...
#if (RTE_USART3 != 0)
#if (RTE_USART3_TX_DMA != 0)
  void DMAx_CHANNELy_EVENT(RTE_USART3_TX_DMA_NUMBER, RTE_USART3_TX_DMA_CHANNEL) (uint32_t event) {
    UART_DMA_TxChannelEvent(event, ptr_UART[2]);
  }
#endif
#endif
...
#if (RTE_UART4 != 0)
#if (RTE_UART4_TX_DMA != 0)
  void DMAx_CHANNELy_EVENT(RTE_UART4_TX_DMA_NUMBER, RTE_UART4_TX_DMA_CHANNEL) (uint32_t event) {
    UART_DMA_TxChannelEvent(event, ptr_UART[3]);
  }
#endif
#endif
...
#if (RTE_UART5 != 0)
#if (RTE_UART5_TX_DMA != 0)
  void DMAx_CHANNELy_EVENT(RTE_UART5_TX_DMA_NUMBER, RTE_UART5_TX_DMA_CHANNEL) (uint32_t event) {
    UART_DMA_TxChannelEvent(event, ptr_UART[4]);
  }
#endif
#endif
