...
static const GPIO_PIN_ID eth_out_pins[] = {
...
/* Ethernet Input Pins */
static const GPIO_PIN_ID eth_in_pins[] = {
#if (RTE_ETH_RMII)
...
static ARM_ETH_STATUS Initialize (ARM_ETH_MAC_SignalEvent_t cb_event,
  ...
  for (pin = eth_out_pins; pin != &eth_out_pins[sizeof(eth_out_pins)/sizeof(GPIO_PIN_ID)]; pin++) {
    GPIO_PortClock   (pin->port, true);
    GPIO_PinConfigure(pin->port, pin->num, GPIO_AF_PUSHPULL, GPIO_MODE_OUT50MHZ);
  }

  /* Configure Ethernet Input pins */
    for (pin = eth_in_pins; pin != &eth_in_pins[sizeof(eth_in_pins)/sizeof(GPIO_PIN_ID)]; pin++) {
    GPIO_PortClock   (pin->port, true);
    GPIO_PinConfigure(pin->port, pin->num, GPIO_IN_FLOATING, GPIO_MODE_INPUT);
  }
...
static ARM_ETH_STATUS Uninitialize (void) {
  ...
  /* Reset Ethernet Output pins */
  for (pin = eth_out_pins; pin != &eth_out_pins[sizeof(eth_out_pins)/sizeof(GPIO_PIN_ID)]; pin++) {
    GPIO_PinConfigure(pin->port, pin->num, GPIO_OUT_PUSH_PULL, GPIO_MODE_INPUT);
  }

  /* Reset Ethernet Input pins */
  for (pin = eth_in_pins; pin != &eth_in_pins[sizeof(eth_in_pins)/sizeof(GPIO_PIN_ID)]; pin++) {
    GPIO_PinConfigure(pin->port, pin->num, GPIO_OUT_PUSH_PULL, GPIO_MODE_INPUT);
  }
...
#if (RTE_ETH_RMII)
  /* Reset MII/RMII selection bit */
  AFIO->MAPR &= ~AFIO_MAPR_MII_RMII_SEL;
#endif

#if (RTE_ETH_REF_CLOCK && RTE_ETH_MII && (RTE_HSE == 25000000))
  /* Disable 25MHz MCO clock for MII interface */
  RCC->CFGR &= ~RCC_CFGR_MCO;
#endif
  return ARM_ETH_OK;
}
