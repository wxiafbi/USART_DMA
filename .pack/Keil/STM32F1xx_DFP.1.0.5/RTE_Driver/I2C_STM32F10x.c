 ...
 * Project:      I2C Driver for ST STM32F10x
 * ---------------------------------------------------------------------- 
...
#define I2C_BUSY_TIMEOUT            1000/* I2C bus busy wait timeout in us        */
#define I2C_BUS_CLEAR_CLOCK_PERIOD  10  /* I2C bus clock period in us             */

#define DMA_CHANNEL_DISABLE_TOUT    100 /* DMA Channel disable timeout in CPU cycles */
...
typedef struct {
        ...
        AFIO_REMAP        pin_remap;    // Pin alternate function remap
...
static const DMA_INFO I2C1_DMA_RX = {
  DMAx_CHANNELy(RTE_I2C1_RX_DMA_NUMBER, RTE_I2C1_RX_DMA_CHANNEL),
...
static const DMA_INFO I2C1_DMA_TX = {
  DMAx_CHANNELy(RTE_I2C1_TX_DMA_NUMBER, RTE_I2C1_TX_DMA_CHANNEL),
...
static const DMA_INFO I2C2_DMA_RX = {
  DMAx_CHANNELy(RTE_I2C2_RX_DMA_NUMBER, RTE_I2C2_RX_DMA_CHANNEL),
...
static const DMA_INFO I2C2_DMA_TX = {
  DMAx_CHANNELy(RTE_I2C2_TX_DMA_NUMBER, RTE_I2C2_TX_DMA_CHANNEL),
...
/* I2C Resources */
static I2C_RESOURCES *ptr_I2C[2] = {
#if (RTE_I2C1)
  &I2C1_Resources,
#else
  NULL,
...
static ARM_I2C_STATUS I2C_Initialize (ARM_I2C_SignalEvent_t cb_event,
  ...
  /* Initialize DMA Streams */
    ...
    if (ptr_i2c->dma_rx) {
      DMA_ChannelInitialize(ptr_i2c->dma_rx->dma, ptr_i2c->dma_rx->channel);
    }
  #endif
    ...
    if (ptr_i2c->dma_tx) {
      DMA_ChannelInitialize(ptr_i2c->dma_tx->dma, ptr_i2c->dma_tx->channel);
    }
  #endif
  ...
  GPIO_AFConfigure(ptr_i2c->pin_remap);
...
static ARM_I2C_STATUS I2C_Uninitialize (I2C_RESOURCES *ptr_i2c) {
  ...
  /* Uninitialize DMA Streams */
       ...
       (RTE_I2C3 && RTE_I2C3_RX_DMA))
    if (ptr_i2c->dma_rx) {
      DMA_ChannelUninitialize(ptr_i2c->dma_rx->dma, ptr_i2c->dma_rx->channel);
    }
  #endif
  #if ((RTE_I2C1 && RTE_I2C1_TX_DMA) || \
       (RTE_I2C2 && RTE_I2C2_TX_DMA) || \
       (RTE_I2C3 && RTE_I2C3_TX_DMA))
    if (ptr_i2c->dma_tx) {
      DMA_ChannelUninitialize(ptr_i2c->dma_tx->dma, ptr_i2c->dma_tx->channel);
    }
  #endif
...
static ARM_I2C_STATUS I2C_PowerControl (ARM_POWER_STATE state, I2C_RESOURCES *ptr_i2c) {
  ...
  switch (state) {
    case ARM_POWER_OFF:
      ...
      /* Disable DMA Streams */
      if (ptr_i2c->dma_tx) {
        DMA_ChannelDisable(ptr_i2c->dma_tx->ptr_channel);
      }
      if (ptr_i2c->dma_rx) {
        DMA_ChannelDisable(ptr_i2c->dma_rx->ptr_channel);
      }
...
static int32_t I2C_SendData (uint32_t       addr,
  ...
  if (ptr_i2c->dma_tx) {
    ...
    s_ctrl = (ptr_i2c->dma_rx->priority << DMA_PRIORITY_POS)|
    ...
    for (i = DMA_CHANNEL_DISABLE_TOUT; i; i++) {
      if (!DMA_ChannelStatus (ptr_i2c->dma_tx->ptr_channel)) break;
    }
    if (DMA_ChannelStatus (ptr_i2c->dma_tx->ptr_channel) == true) {
      return (-ARM_I2C_ERROR);
    }

    DMA_ChannelConfigure (ptr_i2c->dma_tx->ptr_channel,
                         s_ctrl,
                         (uint32_t)&(ptr_i2c->i2c->DR),
                         (uint32_t)data,
                         size);
    DMA_ChannelEnable (ptr_i2c->dma_tx->ptr_channel);
  ...
  if (ptr_i2c->dma_tx) {
    ret_val = size - DMA_ChannelTransferItemCount(ptr_i2c->dma_tx->ptr_channel);
  }
  else {
...
static int32_t I2C_ReceiveData (uint32_t       addr,
  ...
  if (ptr_i2c->dma_rx) {
    ...
    DMA_ChannelConfigure (ptr_i2c->dma_rx->ptr_channel,
                         s_ctrl,
                         (uint32_t)&(ptr_i2c->i2c->DR),
                         (uint32_t)data,
                         size);

    DMA_ChannelEnable (ptr_i2c->dma_rx->ptr_channel);
    ptr_i2c->i2c->CR2 = (ptr_i2c->i2c->CR2 & ~I2C_CR2_ITBUFEN) | I2C_CR2_DMAEN |
                                                                 I2C_CR2_LAST  ;
  }
  else {
  ...
  /* Set received size */
  if (ptr_i2c->dma_rx) {
    ret_val = size - DMA_ChannelTransferItemCount (ptr_i2c->dma_rx->ptr_channel);
  }
  else {
    ret_val = size - ptr_i2c->info->size;
  }
...
#if (RTE_I2C1)
#if (RTE_I2C1_TX_DMA)
void DMAx_CHANNELy_EVENT (RTE_I2C1_TX_DMA_NUMBER, RTE_I2C1_TX_DMA_CHANNEL) (uint32_t event) {
  ptr_I2C[0]->i2c->CR2 &= ~I2C_CR2_DMAEN;
  /* BTF event will be generated after EOT - BTF will release the semaphore */
}
#endif
#if (RTE_I2C1_RX_DMA)
void DMAx_CHANNELy_EVENT (RTE_I2C1_RX_DMA_NUMBER, RTE_I2C1_RX_DMA_CHANNEL) (uint32_t event) {
  ptr_I2C[0]->i2c->CR2 &= ~I2C_CR2_DMAEN;
...
#if (RTE_I2C2)
#if (RTE_I2C2_TX_DMA)
void DMAx_CHANNELy_EVENT (RTE_I2C2_TX_DMA_NUMBER, RTE_I2C2_TX_DMA_CHANNEL) (uint32_t event) {
  ptr_I2C[1]->i2c->CR2 &= ~I2C_CR2_DMAEN;
  /* BTF event will be generated after EOT - BTF will release the semaphore */
}
#endif
#if (RTE_I2C2_RX_DMA)
void DMAx_CHANNELy_EVENT (RTE_I2C2_RX_DMA_NUMBER, RTE_I2C2_RX_DMA_CHANNEL) (uint32_t event) {
  ptr_I2C[1]->i2c->CR2 &= ~I2C_CR2_DMAEN;
  /* No I2C events after this point - release the semaphore */
