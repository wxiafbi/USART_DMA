 ...
 * Project:      SPI Driver for ST STM32F10x
 * ---------------------------------------------------------------------- 
...
#error "SPI not configured in RTE_Device.h!"
     ...
     ( (RTE_SPI1_RX_DMA_NUMBER   != 1)                                     || \
       (RTE_SPI1_RX_DMA_CHANNEL  != 2)                                     || \
...
#error "SPI2 Tx DMA configuration in RTE_Device.h is invalid!"
     ...
     ( (RTE_SPI3_RX_DMA_NUMBER   != 2)                                     || \
       (RTE_SPI3_RX_DMA_CHANNEL  != 1)                                     || \
...
#error "SPI3 Rx DMA configuration in RTE_Device.h is invalid!"
     ...
     ( (RTE_SPI3_TX_DMA_NUMBER   != 2)                                     || \
       (RTE_SPI3_TX_DMA_CHANNEL  != 2)                                     || \
...
typedef struct {
        ...
        AFIO_REMAP        pin_remap_def;        // Default pin remapping setting
        AFIO_REMAP        pin_remap;            // Pin remapping information
...
static const DMA_INFO SPI1_DMA_RX = {
  DMAx_CHANNELy(RTE_SPI1_RX_DMA_NUMBER, RTE_SPI1_RX_DMA_CHANNEL),
...
static const DMA_INFO SPI1_DMA_TX = {
  DMAx_CHANNELy(RTE_SPI1_TX_DMA_NUMBER, RTE_SPI1_TX_DMA_CHANNEL),
...
static SPI_RESOURCES SPI1_Resources = {
  ...
  { NULL },
#endif
  { RTE_SPI1_SCK_PORT,  RTE_SPI1_SCK_BIT  },
...
static const DMA_INFO SPI2_DMA_RX = {
  DMAx_CHANNELy(RTE_SPI2_RX_DMA_NUMBER, RTE_SPI2_RX_DMA_CHANNEL),
...
static const DMA_INFO SPI2_DMA_TX = {
  DMAx_CHANNELy(RTE_SPI2_TX_DMA_NUMBER, RTE_SPI2_TX_DMA_CHANNEL),
...
static SPI_RESOURCES SPI2_Resources = {
  ...
  { NULL },
#endif
  { RTE_SPI2_SCK_PORT,  RTE_SPI2_SCK_BIT  },
...
static const DMA_INFO SPI3_DMA_RX = {
  DMAx_CHANNELy(RTE_SPI3_RX_DMA_NUMBER, RTE_SPI3_RX_DMA_CHANNEL),
...
static const DMA_INFO SPI3_DMA_TX = {
  DMAx_CHANNELy(RTE_SPI3_TX_DMA_NUMBER, RTE_SPI3_TX_DMA_CHANNEL),
...
static SPI_RESOURCES SPI3_Resources = {
  ...
  { NULL },
#endif
  { RTE_SPI3_SCK_PORT,  RTE_SPI3_SCK_BIT  },
...
static ARM_SPI_STATUS SPI_Initialize (ARM_SPI_SignalEvent_t cb_event,
  ...
  *ptr_spi->reset_reg &= ~ptr_spi->reset_mask;
  ...
  if (ptr_spi->dma_rx) {
    DMA_ChannelInitialize(ptr_spi->dma_rx->dma, ptr_spi->dma_rx->channel);
  }
  if (ptr_spi->dma_tx) {
    DMA_ChannelInitialize(ptr_spi->dma_tx->dma, ptr_spi->dma_tx->channel);
  }
  ...
  // Setup pin remapping
  GPIO_AFConfigure(ptr_spi->pin_remap);
...
static ARM_SPI_STATUS SPI_Uninitialize (SPI_RESOURCES *ptr_spi) {
  ...
  *ptr_spi->clock_reg &= ~ptr_spi->clock_mask;
  ...
  if (ptr_spi->dma_rx) {
    DMA_ChannelUninitialize(ptr_spi->dma_rx->dma, ptr_spi->dma_rx->channel);
  }
  if (ptr_spi->dma_tx) {
    DMA_ChannelUninitialize(ptr_spi->dma_tx->dma, ptr_spi->dma_tx->channel);
  }
  ...
  GPIO_AFConfigure(ptr_spi->pin_remap_def);
  ...
  switch (state) {
    case ARM_POWER_OFF:
      ...
      *ptr_spi->clock_reg &= ~ptr_spi->clock_mask;
      ...
      if (ptr_spi->dma_tx) {
        DMA_ChannelDisable(ptr_spi->dma_tx->ptr_channel);
      }
      if (ptr_spi->dma_rx) {
        DMA_ChannelDisable(ptr_spi->dma_rx->ptr_channel);
      }
...
static ARM_SPI_STATUS SPI_SendData (const uint8_t  *buf,
  ...
  if (ptr_spi->dma_rx) {
    DMA_ChannelConfigure(
      ...
      (uint32_t)&ptr_spi->spi->DR,              // Peripheral Address
      (uint32_t)&ch,                            // Memory Address
      len                                       // Number of Data items to transfer
    );
    DMA_ChannelEnable(ptr_spi->dma_rx->ptr_channel);
  ...
  if (ptr_spi->dma_tx) {
    ...
    DMA_ChannelEnable(ptr_spi->dma_tx->ptr_channel);
...
#if (RTE_SPI1)
#if (RTE_SPI1_TX_DMA)
void DMAx_CHANNELy_EVENT(RTE_SPI1_TX_DMA_NUMBER, RTE_SPI1_TX_DMA_CHANNEL) (uint32_t events) {
  SPI_DMA_TxComplete(&SPI1_Resources);
}
#endif
#if (RTE_SPI1_RX_DMA)
void DMAx_CHANNELy_EVENT(RTE_SPI1_RX_DMA_NUMBER, RTE_SPI1_RX_DMA_CHANNEL) (uint32_t events) {
...
#if (RTE_SPI2)
#if (RTE_SPI2_TX_DMA)
void DMAx_CHANNELy_EVENT(RTE_SPI2_TX_DMA_NUMBER, RTE_SPI2_TX_DMA_CHANNEL) (uint32_t events) {
  SPI_DMA_TxComplete(&SPI2_Resources);
}
#endif
#if (RTE_SPI2_RX_DMA)
void DMAx_CHANNELy_EVENT(RTE_SPI2_RX_DMA_NUMBER, RTE_SPI2_RX_DMA_CHANNEL) (uint32_t events) {
...
#if (RTE_SPI3)
#if (RTE_SPI3_TX_DMA)
void DMAx_CHANNELy_EVENT(RTE_SPI3_TX_DMA_NUMBER, RTE_SPI3_TX_DMA_CHANNEL) (uint32_t events) {
  SPI_DMA_TxComplete(&SPI3_Resources);
}
#endif
#if (RTE_SPI3_RX_DMA)
void DMAx_CHANNELy_EVENT(RTE_SPI3_RX_DMA_NUMBER, RTE_SPI3_RX_DMA_CHANNEL) (uint32_t events) {
